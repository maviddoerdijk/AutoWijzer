...
from modules.pydantic_models import UserData
load_dotenv()
...
def render_page():
    ...
    if st.session_state.current_step == 0:        
        ...
        if budget_type == "Maandelijks":
            ...
            st.session_state.user_data.budget_type = "Maandelijks"
            st.session_state.user_data.budget_min = min_budget_monthly
            st.session_state.user_data.budget_max = max_budget_monthly
        else:
            ...
            st.session_state.user_data.budget_type = "Cash"
            st.session_state.user_data.budget_min = min_budget_cash
            st.session_state.user_data.budget_max = max_budget_cash
        ...
        st.subheader(f"2.1 Aanvullende vragen over {st.session_state.user_data.fuel_type}")
        # Vraag 3: Vervolgvragen op basis van brandstoftype
        fuel_type = st.session_state.user_data.fuel_type
        ...
        st.subheader(f"2.2 Aanvullende vragen over {st.session_state.user_data.fuel_type}")
...
def render_result_page():    
    ...
    subheader_placeholder.subheader("🔍 We zijn bezig met het vinden van de perfecte auto voor u...")

    # OpenAI functie aanroepen om een auto te vinden
    with st.spinner("Auto gegevens worden opgehaald..."):
        car_data = get_car_data(st.session_state.user_data)
    
    # Update subheader once data is fetched
    subheader_placeholder.subheader("🎉 Gefeliciteerd! We hebben de perfecte auto voor u gevonden!)")
    ...
    with col1:
        ...
        st.image(aux_image, caption=car_data.full_name, use_column_width=True)
